...
angular.module('shoplyApp')
  .controller('SalidaCtrl', function ($scope, $timeout, $rootScope,  sweetAlert, constants, $state, modal, api, storage) {
  	$scope.Records = false;

      $scope.load = function(){
        api.salidas().get().success(function(res){
          $scope.records = res || [];
          $scope.Records = true;
        });
    	}
  	...
  	$scope.create = function(){
       window.modal = modal.show({templateUrl : 'views/salidas/agregar_salida.html', size :'lg', scope: $scope, backdrop:'static'}, function($scope){
           if($scope.formCreateOutput.$invalid){
            ...
            api.salidas().post($scope.form).success(function(res){
              if(res){
    ...
    $scope.edit = function(){
      ...
      window.modal = modal.show({templateUrl : 'views/entradas/editar_entrada.html', size :'md', scope: $scope, backdrop:'static'}, function($scope){
            ...
            api.salidas($scope.formEdit._id).put($scope.formEdit).success(function(res){
                if(res){
    ...
    $scope.delete = function(record){
        ...
        modal.removeConfirm({closeOnConfirm : true}, 
            function(isConfirm){ 
               if (isConfirm) {
                    api.salidas(_record._id).delete().success(function(res){
                        if(res){
...
angular.module('shoplyApp')
  .controller('SalidaCreateCtrl', function ($scope, $timeout, $rootScope,  sweetAlert, constants, $state, modal, api, storage, shoppingCart) {
